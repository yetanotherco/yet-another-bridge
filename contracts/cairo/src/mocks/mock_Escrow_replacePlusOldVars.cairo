...
#[starknet::interface]
trait IEscrow_replacePlusOldVars<ContractState> {
    ...
    fn get_mm_ethereum_contract_V2(self: @ContractState) -> EthAddress;
    fn get_mm_starknet_contract_V2(self: @ContractState) -> ContractAddress;
    fn get_mm_ethereum_contract_old(self: @ContractState) -> EthAddress;
    fn get_mm_starknet_contract_old(self: @ContractState) -> ContractAddress;
    
    fn set_eth_transfer_contract(ref self: ContractState, new_contract: EthAddress);
    fn set_mm_ethereum_contract_V2(ref self: ContractState, new_contract: EthAddress);
    fn set_mm_starknet_contract_V2(ref self: ContractState, new_contract: ContractAddress);
}

#[starknet::contract]
mod Escrow_replacePlusOldVars {
    use super::{IEscrow_replacePlusOldVars, Order};
    ...
    struct Storage {
        ...
        mm_ethereum_wallet_V2: EthAddress,
        mm_starknet_wallet_V2: ContractAddress,
    ...
    fn constructor(
        ...
        self.mm_ethereum_wallet_V2.write(mm_ethereum_wallet_V2);
        self.mm_starknet_wallet_V2.write(mm_starknet_wallet_V2);
    ...
    #[external(v0)]
    impl Escrow of IEscrow_replacePlusOldVars<ContractState> {
        fn get_order(self: @ContractState, order_id: u256) -> Order {
        ...
        fn get_mm_ethereum_contract_V2(self: @ContractState) -> EthAddress {
            self.mm_ethereum_wallet_V2.read()
        }

        fn get_mm_starknet_contract_V2(self: @ContractState) -> ContractAddress {
            self.mm_starknet_wallet_V2.read()
        }
        ...
        fn set_mm_ethereum_contract_V2(ref self: ContractState, new_contract: EthAddress) {
            self.ownable.assert_only_owner();
            self.mm_ethereum_wallet_V2.write(new_contract);
        }

        fn set_mm_starknet_contract_V2(ref self: ContractState, new_contract: ContractAddress) {
            self.ownable.assert_only_owner();   
            self.mm_starknet_wallet_V2.write(new_contract);
        }
    }
    ...
    fn withdraw(
        ...
        IERC20Dispatcher { contract_address: self.native_token_eth_starknet.read() }
            .transfer(self.mm_starknet_wallet_V2.read(), payment_amount);

        self.emit(Withdraw { order_id, address: self.mm_starknet_wallet_V2.read(), amount });
    }
}
